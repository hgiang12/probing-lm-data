...
from peft import PeftModel  # << add this!
...
def read_jsonl(path):
    ...
    return [{"text": d["text"], "label": d["label"]} for d in data]
...
def get_acts(statements, tokenizer, model, layers):
    ...
    for layer in acts:
        acts[layer] = torch.stack(acts[layer]).float()
...
if __name__ == "__main__":
    ...
    model = model.merge_and_unload()  # optional, can skip if you want faster loading
